        ...
        try {
            if (!NetUtil.isNetworkAvailable())
                throw new NetworkError("The network is not available, please check the network. The requested url is:" +
                        " " + url);
//            Intent intent = new Intent((Context) mExecutor, ErrorActivity.class);
//            Context.startActivity(intent);
//            mContext.startActivity(new Intent(mContext, WithdrawalActivity.class).putExtra("payType", item.getBillType()));//这里有的
