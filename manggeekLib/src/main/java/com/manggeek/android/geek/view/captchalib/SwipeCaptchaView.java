...
import static com.manggeek.android.geek.view.captchalib.DrawHelperUtils.drawPartCircle;
...
public class SwipeCaptchaView extends ImageView {
    ...
    private void setEmbossMaskFilter() {
        ...
        float specular = 5;
        float blur = 5;
        mMaskPaint.setMaskFilter(new EmbossMaskFilter(direction, ambient, specular, blur));
    }
...
//                Log.d(TAG, "onAnimationUpdate: " + animatedValue);
    ...
    private void createCaptchaPath() {
        ...
        //随机生成验证码阴影左上角 x y 点，
        //保证阴影部分在图片的右半部分
        mCaptchaX = mRandom.nextInt(mWidth / 2 - mCaptchaWidth - gap) + mWidth / 2;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isMatchMode) {
            ...
            if (mCaptchaPath != null) {
                canvas.drawPath(mCaptchaPath, stockPaint);
